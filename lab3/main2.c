...
void calc(struct sh_mem_t* sh_mem, int proc, int proc_n, int sems){
    int i = proc_n;
    while(i < sh_mem->matrix.cols){
        for(int j = 0; j < sh_mem->matrix.rows; ++j){
            struct sembuf buf;
            buf.sem_num = j;
            buf.sem_op = 1;
            buf.sem_flg = 0;
            semop(sems, &buf, 1);
            sh_mem->result.e[i] += sh_mem->matrix.e[sh_mem->matrix.cols * j + i] * sh_mem->vector.e[j];
            buf.sem_op = -1;
            semop(sems, &buf, 1);
        }
        i += proc;
...
//    printf("alloc4\n");
    ...
    int sems = semget(IPC_PRIVATE, m, 0666 | IPC_CREAT | IPC_EXCL);
    for(int i = 0; i < proc; ++i){
        printf("before fork()\n");
        int pid = fork();
        if(pid == 0){
            calc(sh_mem, proc, i, sems);
            return 0;
        }
    }
    for(int i = 0; i < proc; ++i){
